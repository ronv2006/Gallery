...
int MemoryAccess::getLastIdOfUser()
{
	return 0;
}

std::vector<User> MemoryAccess::getUsers()
{
	return std::vector<User>();
}

int MemoryAccess::getLastIdOfPicture()
{
	return 0;
}
...
void MemoryAccess::deleteUser(const User& user)
{
	if (doesUserExists(user.getId())) {
		...
		std::list<Album> aList = getAlbumsOfUser(user);
