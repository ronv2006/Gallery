    ...
    m_dataAccess(dataAccess)
{
	// Left empty
	m_dataAccess.open();
	this->m_nextUserId = dataAccess.getLastIdOfUser();
	this->m_nextPictureId = dataAccess.getLastIdOfPicture();
...
void AlbumManager::userStatistics()
{
	...
	std::cout << "user @" << userId << " Statistics:" << std::endl << "--------------------" << std::endl <<
		...
		"  + Number of albums user has: " << m_dataAccess.countAlbumsOwnedOfUser(user) << std::endl;
}
